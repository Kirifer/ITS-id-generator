...
router.get('/by-id-number/:idNumber', async (req, res) => {
  try {
    ...
    if (!item) return res.status(404).json({ message: 'No approved ID with that number' });
...
router.post('/', verifyToken, requireRole('Admin'), upload.single('photo'), async (req, res) => {
  try {
    ...
    // Required fields
    const required = { firstName, lastName, idNumber, position, type, emFirstName, emLastName, emPhone };
...
/* =========================
   ...
   ========================= */
router.get('/', verifyToken, requireAnyRole('Admin', 'Approver'), async (req, res) => {
  try {
...
/* =========================
   ...
   ========================= */
router.patch('/:id/approve', verifyToken, requireRole('Approver'), async (req, res) => {
  try {
    const { id } = req.params;
    if (!mongoose.isValidObjectId(id)) return res.status(400).json({ message: 'Invalid ID' });

    const updated = await IdCard.findByIdAndUpdate(id, { status: 'Approved' }, { new: true });
...
router.patch('/:id/reject', verifyToken, requireRole('Approver'), async (req, res) => {
  try {
    const { id } = req.params;
    if (!mongoose.isValidObjectId(id)) return res.status(400).json({ message: 'Invalid ID' });

    const updated = await IdCard.findByIdAndUpdate(id, { status: 'Rejected' }, { new: true });
    if (!updated) return res.status(404).json({ message: 'Not found' });
    res.json(updated);
  } catch (e) {
    res.status(500).json({ message: e.message });
...
router.delete('/:id', verifyToken, requireRole('Admin'), async (req, res) => {
  try {
    ...
    // Best-effort cleanup
    const toDelete = [];
    if (doc.photoPath) toDelete.push(path.join(__dirname, '..', doc.photoPath.replace(/^\//, '')));
    if (doc.generatedImagePath) toDelete.push(path.join(__dirname, '..', doc.generatedImagePath.replace(/^\//, '')));
