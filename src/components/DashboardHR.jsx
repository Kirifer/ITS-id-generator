...
export default function Dashboard() {
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden">
      ...
      {/* Sidebar */}
      <aside className={` ${selectedId ? 'w-20' : 'w-60'} bg-[#262046] text-white min-h-screen p-5 flex flex-col justify-between transition-all duration-300`}>
      ...
      {/* Main Content */}
      <main className={`flex-1 p-6 transition-all duration-300 ${selectedId ? 'mr-[600px]' : ''} custom-bg flex flex-col overflow-hidden`}>
        {/* Stat Card */}
                ...
                <tbody>
                  {filteredData.map((id, index) => (
                    <tr key={index} className="bg-white even:bg-gray-100">
                      ...
                      <td className="p-4 text-purple-600 cursor-pointer flex items-center justify-center" onClick={() => setSelectedId(id)}>
                        <Eye size={16} className="hover:border-b hover:border-purple-800 transition-all duration-200" />
                      </td>
                    </tr>
                  ))}
